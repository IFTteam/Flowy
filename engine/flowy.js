var flowy = function(canvas, grab, release, snapping, rearrange, spacing_x, spacing_y) {
    ...
    flowy.load = function() {
        ...
        flowy.endDrag = function(event) {
            ...
            if (event.which != 3 && (active || rearrange)) {
                ...
                if (parseInt(drag.querySelector(".blockid").value) === 0 && rearrange) {
                    ...
                    console.log(blocks, drag, blocko);
                    // const checker = ['10', '11', '12']
                    for (var i = 0; i < blocks.length; i++) {
                        //console.log('check attach', checkAttach(blocko[i]));
                        //edited here
                        //original ------ if (checkAttach(blocko[i]) ) {
                            // && !checker.includes(drag.querySelector(".blockelemtype").value)
                        if (checkAttach(blocko[i]) && blocks.filter(id => id.parent == blocko[i]).length <= 0) {
        ...
        function hasParentClass(element, classname) {
            //console.log('check if has parent class');
