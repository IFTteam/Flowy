var flowy = function(canvas, grab, release, snapping, rearrange, spacing_x, spacing_y) {
    ...
    flowy.load = function() {
        ...
        flowy.import = function(output) {
            //console.log('flowy import function')
            ...
            for (var a = 0; a < output.blockarr.length; a++) {
                blocks.push({
                    ...
                    typeid: drag.querySelector(".blockelemtype").value
                })
            }
            if (blocks.length > 1) {
                rearrangeMe();
        ...
        flowy.zoom = function() {
            ...
            const panzoom = Panzoom(elem, {maxScale: 5, excludeClass: 'blockyright'});
        ...
        flowy.endDrag = function(event) {
            ...
            if (event.which != 3 && (active || rearrange)) {
                ...
                if (parseInt(drag.querySelector(".blockid").value) === 0 && rearrange) {
                    ...
                    console.log('blocks', blocks, blocko,document.querySelector(".blockid[value='" + blocko[i] + "']"));
                    for (var i = 0; i < blocks.length; i++) {
                        if (checkAttach(blocko[i]) 
                        && !arr.includes(blocks[blocko[i]].typeid)
                        && blocks.filter(id => id.parent == blocko[i]).length <= 0) {
                            //console.log('check if is the right blocks', blocks[blocko[i]]);
                            active = false;
                            const variable = document.querySelector(".blockid[value='" + blocko[i] + "']");
                            ...
                            break;
                        } else if(checkAttach(blocko[i]) 
                        && arr.includes(blocks[blocko[i]].typeid)
                        && blocks.filter(id => id.parent == blocko[i]).length <= 1) {
                            active = false;
                            const variable = document.querySelector(".blockid[value='" + blocko[i] + "']");
                            if (blockSnap(drag, false, document.querySelector(".blockid[value='" + blocko[i] + "']").parentNode)  ) {
                                snap(drag, i, blocko);
            ...
            // console.log('check element inside',element, block);
            // const panzoom = Panzoom(block, {});
        ...
        function removeSelection() {
            console.log('remove the selection');
            canvas_div.appendChild(document.querySelector(".indicator"));
            drag.parentNode.removeChild(drag);
        }
        
        function firstBlock(type) {
            console.log('check if it is first block');
            if (type == "drop") {
                console.log('drop at the position');
                ...
                console.log('add the property to the block', blocks[0].x);
            } else if (type == "rearrange") {
                drag.classList.remove("dragging");
                console.log('rearrange at the position');
                rearrange = false;
        ...
        function drawArrow(arrow, x, y, id) {
            console.log('draw the arr to the next');
            if (x < 0) {
        ...
        function updateArrow(arrow, x, y, children) { 
            console.log('check if need to update the arrow');
            console.log(x);
            if (x < 0) {
        ...
        function snap(drag, i, blocko) {
            //console.log('snap them together', blocks.filter(id => id.parent == blocko[i]).length);
            if (!rearrange) {
                canvas_div.appendChild(drag);
            }
            var totalwidth = 0;
        ...
        function touchblock(event) {
            //console.log('touch the block');
            dragblock = false;
            if (hasParentClass(event.target, "block")) {
                var theblock = event.target.closest(".block");
                //console.log(event.target, theblock);
                if (event.targetTouches) {
